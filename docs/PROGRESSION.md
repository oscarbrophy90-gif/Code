...
**Currency buys cosmetics and attributes. Money buys cosmetics.** There is no path that
...
A build's body determines what it can become. Sixteen attributes each get a hard ceiling
...
rebounding  = 62 + h·28 + w·14 + span·8
block       = 60 + h·28 + span·12
interiorD   = 66 + h·24 + w·14 + span·6
…
```

Position then applies a modest bias (a PG gets +5 Ball Handle and −6 Rebounding, a C gets
+6 Block and −8 Ball Handle), so position choice matters without any one position
dominating.
...
Point 1.35; a C weights Rebounding 1.5 and Interior Defense 1.45), then the useful band is
mapped onto 60–99:
...
A fresh build starts each attribute 28% of the way to its cap, which lands every build at
60–63 overall — weak everywhere but already shaped by its body. A fully maxed focused
build lands in the mid 90s.
...
Measured on an SG build (6'6", 205 lb, 6'10" wingspan):

| | Cost | At ~2,000 CC/game |
| --- | --- | --- |
| Max eight core attributes (62 → ~93 OVR) | 409,520 CC | ~205 games |
| Max all sixteen (62 → 97 OVR) | 601,020 CC | ~300 games |
...
4, Rooftop at 8) and act as a general account-age signal. They do not gate ratings.
...
Ranked pays 1.3×, events 1.15×, and private matches 0.25× so friends can play without the
mode becoming a farm. Everything is then scaled by
`lengthFactor = clamp(duration / 150s, 0.25, 1)`, which is what stops a 30-second quit
from paying like a full game.

A competitive win lands around 1,800–2,600 CC and a similar amount of XP.
...
At the roll: the battle pass resets, ranked points soft-reset to 62% with three placement
games, and season-high is preserved. Ranked rewards are granted on **highest rank reached**,
not final standing, so a late losing streak never costs you what you already earned.
...
Three dailies (450–800 CC), three weeklies (2,200–4,000 CC) and four seasonal objectives
(9,000–15,000 CC).
...
4. Ranked matchmaking bands on rating, not on overall, so a maxed build meets other maxed
   builds rather than farming new players.
