...
export const ATTRIBUTE_META: Record<AttributeKey, AttributeMeta> = {
  layup: { key: 'layup', label: 'Layup', group: 'finishing', blurb: 'Touch and control on close-range finishes.' },
  ...
  passing: { key: 'passing', label: 'Passing', group: 'playmaking', blurb: 'Pass speed and accuracy in team modes.' },
  ...
  rebounding: { key: 'rebounding', label: 'Rebounding', group: 'defense', blurb: 'Board tracking, boxing out, tip range.' },
...
const POSITION_WEIGHTS: Record<Position, Partial<Record<AttributeKey, number>>> = {
  PG: { ballHandle: 1.5, passing: 1.3, speed: 1.3, acceleration: 1.2, threePoint: 1.35, midRange: 1.05, layup: 1.05, steal: 1.15, perimeterDefense: 1.15, stamina: 1.05 },
  SG: { threePoint: 1.5, midRange: 1.25, ballHandle: 1.2, speed: 1.15, acceleration: 1.1, layup: 1.1, perimeterDefense: 1.2, steal: 1.05, dunk: 1.05, stamina: 1.05 },
  SF: { threePoint: 1.25, midRange: 1.15, layup: 1.2, dunk: 1.2, ballHandle: 1.05, perimeterDefense: 1.2, interiorDefense: 1.05, rebounding: 1.1, strength: 1.1, vertical: 1.1 },
  PF: { dunk: 1.35, layup: 1.2, rebounding: 1.35, interiorDefense: 1.3, strength: 1.3, block: 1.2, vertical: 1.15, midRange: 1.05, threePoint: 0.95 },
  C: { dunk: 1.4, rebounding: 1.5, interiorDefense: 1.45, block: 1.4, strength: 1.4, layup: 1.15, vertical: 1.1, threePoint: 0.8, ballHandle: 0.8, speed: 0.85 },
};
...
export function computeCaps(build: BuildSpec): Attributes {
  ...
  set('passing', 99 - h * 16 - Math.max(0, w) * 6);
  ...
  set('rebounding', 62 + h * 28 + w * 14 + span * 8);
  ...
  const bias: Record<Position, Partial<Record<AttributeKey, number>>> = {
    PG: { ballHandle: 5, passing: 5, speed: 3, rebounding: -6, interiorDefense: -5 },
    SG: { threePoint: 4, midRange: 3, steal: 2, rebounding: -4, strength: -3 },
    SF: { layup: 3, perimeterDefense: 3, dunk: 2 },
    PF: { rebounding: 4, interiorDefense: 4, strength: 3, ballHandle: -4, threePoint: -3 },
    C: { block: 6, rebounding: 5, interiorDefense: 5, ballHandle: -8, speed: -4, threePoint: -6 },
  };
...
/** Starting attribute spread for a fresh build (a 60 overall rookie). */
export function startingAttributes(build: BuildSpec): Attributes {
  ...
  for (const key of ATTRIBUTE_KEYS) {
    // A rookie starts a little way toward the cap, which lands every build at
    // roughly 60 overall — weak everywhere, but already shaped by the body.
    attrs[key] = clamp(Math.round(MIN_ATTRIBUTE + (caps[key] - MIN_ATTRIBUTE) * 0.28), MIN_ATTRIBUTE, caps[key]);
  }
  return attrs;
}
