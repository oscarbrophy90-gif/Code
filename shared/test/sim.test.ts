...
import type { BuildSpec } from '../src/types.ts';
...
test('perfect timing is an automatic make when open', () => {
  const attrs = startingAttributes({ position: 'SG', heightIn: 77, weightLb: 200, wingspanIn: 80 });
  attrs.threePoint = 90;
...
test('a better rating produces a wider green window', () => {
  const build: BuildSpec = { position: 'SG', heightIn: 77, weightLb: 200, wingspanIn: 80 };
...
test('build caps give each body type a distinct identity', () => {
  const guard = computeCaps({ position: 'PG', heightIn: 72, weightLb: 175, wingspanIn: 75 });
  const big = computeCaps({ position: 'C', heightIn: 86, weightLb: 270, wingspanIn: 92 });
  assert.ok(guard.ballHandle > big.ballHandle + 20);
  assert.ok(guard.threePoint > big.threePoint + 20);
  assert.ok(big.rebounding > guard.rebounding + 20);
  assert.ok(big.block > guard.block + 20);
  assert.ok(big.interiorDefense > guard.interiorDefense);
});

test('a fresh build starts at 60 overall and can reach 99', () => {
  const build: BuildSpec = { position: 'SG', heightIn: 78, weightLb: 205, wingspanIn: 82 };
  const start = startingAttributes(build);
  const overall = computeOverall(start, build.position);
  assert.ok(overall >= 60 && overall <= 72, `expected a rookie overall, got ${overall}`);
